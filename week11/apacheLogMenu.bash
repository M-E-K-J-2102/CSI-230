...
function displayOnlyPages(){
	cat "$logFile" | awk '{print $7}' | sort | uniq -c | awk '{print $1, $2}'
...
function frequentVisitors(){
	visitors=$(cat "$logFile" | cut -d ' ' -f 1 | sort -n | uniq -c | awk '$1 > 10 {print $2, $1}')
	echo "$visitors"
}
...
function suspiciousVisitors(){
	:> susVisit.txt
# TODO: check IFS and keyword exist as variables. Find how to 
# do a for each keyword in ioc.txt loop
	while IFS= read -r keyword; do
	   if [[ -n "keyword" ]]; then
		grep "$keyWord" "$logFile" | cut -d ' ' -f 1 >> susVisit.txt
	   fi
	done < ioc.txt

	sort susVisit.txt | uniq -c | awk '$1 > 0 {print $2, $1}'
}
